...
#remove empty rows 
empty_rows <- dfm_subset(covariate_dfm, ntoken(blogs_dfm) == 0)
...
avg_topic_amts <- function(idx){
  ...
  topic_dist <- tidy(news_topic_model, matrix = 'gamma')[which(topic_dist$document %in% idx),]
...
#I was just testing it out to make sure it works, but I'm thinking 6 different groups for our final analysis. sex count * age count
...
topic_contributions <- t(data.frame(guessed_topic=c('Topic 1','Topic 3', 'Topic 2'), male=male_avgs, female=female_avgs))

print(data.frame(topic_contributions))
...
#age_category <- as.numeric(as.factor(blog_corpus$documents$age_category))
#gender <- as.numeric(as.factor(blog_corpus$documents$gender))
...
beta <- as.data.frame(stm_model$beta)
